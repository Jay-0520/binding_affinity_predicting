...
def _set_up_logging(self, null: bool = False) -> None:
    ...
    self._logger = logging.getLogger(f"{str(self)}_{next(self.__class__.class_count)}")
    self._logger.propagate = False
    self._logger.setLevel(logging.DEBUG)
    # For the file handler, we want to log everything
    file_handler = logging.FileHandler(f"{self.base_dir}/{self.__class__.__name__}.log")
