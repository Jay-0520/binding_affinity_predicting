...
from binding_affinity_predicting.components.autocorrelation import (
    get_statistical_inefficiency as _get_statistical_inefficiency,
)
from binding_affinity_predicting.components.gromacs_orchestration import LambdaWindow
...
class GradientData:
    ...
    def __init__(
        self,
        lam_winds: list["LambdaWindow"],  # noqa: F821
        equilibrated: bool,
        ...
        for lam in lam_winds:
            ...
            run_nos: list[int] = lam._get_valid_run_nos(run_nos=run_nos)
            ...
            var_intra = np.mean(vars_intra)
            squared_sem_intra = np.mean(squared_sems_intra) / len(run_nos)
            stat_ineff = np.mean(stat_ineffs_wind)

            # Get inter-run quantities
            squared_sem_inter = np.var(means_intra) / len(run_nos)
            mean_overall = np.mean(means_intra)
            ...
            sem_intra = np.sqrt(squared_sem_intra)
            sem_inter = np.sqrt(squared_sem_inter)
            gradients_all_winds.append(np.array(gradients_wind))
        ...
        stat_ineffs_all_winds = (
            np.array(stat_ineffs_all_winds) * lam_winds[0].sims[0].timestep
        )  # Timestep should be same for all sims

        # Get the SEMs of the free energy changes from the inter-run SEMs of the gradients
        lam_weights = np.array([lam.lam_val_weight for lam in lam_winds])
        sems_inter_delta_g = np.array(sems_inter_all_winds) * lam_weights

        # Get the times
        if equilibrated:
            start_times = np.array([win._equil_time for win in lam_winds])
        else:
            start_times = np.array([0 for win in lam_winds])
        end_times = np.array(
            [win.sims[0].tot_simtime for win in lam_winds]
        )  # All sims at given lam run for same time
        times = [
            np.linspace(start, end, len(gradients[0]) + 1)[1:]
    ...
    def get_time_normalised_sems(
        ...
        total_times_all_runs = np.array(self.total_times) * len(self.run_nos)  # type: ignore
        sems *= np.sqrt(total_times_all_runs)  # type: ignore
        ...
        smoothened_sems = np.array(smoothened_sems)
    ...
    def get_integrated_error(
        ...
        """
        Calculate the integrated standard error of the mean or root variance (or std) of the gradients
        ...
        if er_type == "sem":
            ...
            y_vals = np.sqrt(self.vars_intra)
        n_vals = len(x_vals)
        ...
        integrated_errors = np.array(integrated_errors)
    ...
    def calculate_optimal_lam_vals(
        self,
        er_type: str = "sem",
        delta_er: Optional[float] = None,
        n_lam_vals: Optional[int] = None,
        ...
        or root variance (i.e., std) as a function of lambda, using the trapezoidal rule.

        "del_H / del_λ" is the gradient here
        "thermodynamic length" -> del_s = np.sqrt(Var(del_H / del_λ)) * del_λ
        so we must have an equal "thermodynamic length" between λ -> Intuitively, regions where
        ∂H/∂λ fluctuates a lot (large Var) should get smaller λ-steps, and vice versa.
        ...
        for requested_sem in requested_sem_vals:
            optimal_lam_val = np.interp(requested_sem, integrated_errors, self.lam_vals)
            ## Note that rounding can result in slight decreases of the
            ## performance compared to the theoretical maximum
            if round_lams:
                optimal_lam_val = np.round(optimal_lam_val, 3)
            optimal_lam_vals.append(optimal_lam_val)

        optimal_lam_vals = np.array(optimal_lam_vals)
        self._optimal_lam_vals = optimal_lam_vals
        return optimal_lam_vals

    def get_predicted_overlap_mat(self, temperature: float = 298) -> np.ndarray:
        """
        ...
        predicted_overlap_mat = np.zeros((self.n_lam, self.n_lam))
    ...
    def get_predicted_improvement_factor(
        ...
        initial_lam_vals: Optional[list[float]] = None,
    ) -> float:
        """
        ...
        if er_type == "sem":
            ...
            errors = np.sqrt(self.vars_intra)
        else:
            raise ValueError("er_type must be either 'sem' or 'root_var'")

        # Calculate the optimal variance, e.g. thermodynamic length squared
        v_opt = (np.trapz(errors, self.lam_vals) ** 2) / len(initial_lam_vals)
