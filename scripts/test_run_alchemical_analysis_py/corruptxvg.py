...
def removeCorruptLines(inputFN, outputFN='output.xvg', headerLine=None, fail=10.0):
    ...
    if difLength == 0:
        print("No corrupt lines found, no data was removed from", inputFN)
    elif difLength >= failing:
        DataRemovalError = Exception(
            "%i corrupted lines of %i found, that is more than %.1f percent. Removal failed, examine input file  %s"
            % (difLength, dataLength, fail, inputFN)
        )
        ...
        print("%i corrupt lines of %i found and removed from %s") % (
            difLength,
            dataLength,
            inputFN,
        )
