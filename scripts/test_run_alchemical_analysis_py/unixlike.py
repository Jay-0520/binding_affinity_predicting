...
def workingOnFileObject(func):
    def wrapper(infile, *args, **kwargs):
        close_after = False
        if isinstance(infile, str):
            # open in BINARY mode so that f.seek(..., 2) works
            f = open(infile, 'rb')
            close_after = True
        else:
            f = infile
...
def tailPy(f, nlines, LENB=1024):
    ...
    # we read backwards, so reverse and join
    data = b"".join(reversed(chunks))
    # decode to str (you can change encoding if needed)
    text = data.decode('utf-8', errors='replace')
    # now split into lines and take the last nlines
    return text.splitlines()[-nlines:]
